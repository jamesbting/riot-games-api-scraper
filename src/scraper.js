...
class Scraper {
    constructor(apiKey, seedPlayer) {
        ...
        request(`${GET_ACCOUNT_BY_RIOT_ID_URL}${this.currentPlayer.gameName}/${this.currentPlayer.tagLine}?api_key=${this.apiKey}`).catch((err)=> {throw err}).then((body) => {
            this.currentPlayer.puuid = JSON.parse(body).puuid
            console.log(this.currentPlayer.puuid)
        })
    }
